...
# SKILL_CN.md v2.0 — 国内信源收集技能（Trust-Source）
...
1. 查 `domestic_sources.yaml` → 该数据是否有官方HTTP API？
   - 有 → webfetch调用API获取结构化数据（标注`[A-api]`）
   - **若有Python环境且已安装akshare** → 可用AKShare快捷路径（标注`[A-ak]`），等价于步骤1
   - 无API → 进入步骤2
...
| macro_china_money_supply | M0/M1/M2 | 央行 | 月度 | site:pbc.gov.cn |
| macro_china_trade_balance | 贸易差额 | 海关总署 | 月度 | site:customs.gov.cn |
| macro_china_fx_reserves_yearly | 外汇储备 | 外管局 | 年度 | site:safe.gov.cn |
| macro_china_lpr | LPR利率 | 央行 | 月度 | site:pbc.gov.cn |
| macro_china_new_financial_credit | 新增社融 | 央行 | 月度 | site:pbc.gov.cn |
| macro_china_gdzctz | 固定资产投资 | 国家统计局 | 月度 | L1统计局V2.0 |
| macro_china_urban_unemployment | 城镇失业率 | 国家统计局 | 月度 | L1统计局V2.0 |
| macro_china_exports_yoy | 出口同比 | 海关 | 月度 | site:customs.gov.cn |
| macro_china_imports_yoy | 进口同比 | 海关 | 月度 | site:customs.gov.cn |
...
| stock_zh_a_hist | A股历史行情 | 东方财富 | 日度 | site:eastmoney.com |
...
| DailyHotApi | Vercel自部署 | 20+平台热榜 | 高 | **待部署**（尚未部署，Agent勿尝试调用） |
