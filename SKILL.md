...
|------|-----------------|-------------------|
| 方法 | GitHub Actions定时抓取 + RSSHub即时 | HTTP API(统计局/世界银行) + webfetch + 限定搜索（AKShare为可选快捷） |
...
**跨境矛盾**：当海外信源与国内信源对同一经济指标/政策给出方向性矛盾时（如海外报道"中国出口下滑"vs国内统计局"出口增长"），同样执行上述Step 1-4模板，并在Step 3中增加"统计口径差异"维度的说明（例如：海外引用出口目的国数据，国内引用海关统计；海外按美元计价，国内按人民币计价），标记 `[跨境矛盾]`。
...
1. **国内/海外路由判断**：
   ...
   > **交叉验证标注规则**：海外部分沿用本Skill的标注惯例（`[CDN]`/`[RSSHub]`/`[实时搜索]`），国内部分统一使用 SKILL_CN.md v2.1 的统一标注体系（`[A-api]`/`[A-ak]`/`[A-web]`/`[A-src]`等8种）
...
0.5 **路由查找与验证**：
   a) 先查 `references/rsshub_routes.md`（本地速查表，命中直接使用）
   b) 未命中 → `online_search` 搜索 `site:docs.rsshub.app {目标站点名}` 实时验证
   c) 都未找到 → 告知用户路由可能不存在或过时，同时用online_search补充
1. 调用 `{RSSHUB_BASE_URL}/路由?format=json`
2. 将RSSHub结果与引擎A数据**合并分析**
3. 所有RSSHub来源条目标注 `[RSSHub]` + 原始链接
