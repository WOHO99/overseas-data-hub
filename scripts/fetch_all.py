#!/usr/bin/env python3
"""
fetch_all.py v4.6 — 主调度脚本 (subprocess隔离 + 并发模块 + Bucket+RapidFuzz去重)
...
v4.6: GNews RSS功能利用率优化
  - gnews_url: num=100(默认)+when=7d(默认)+topic模式
  - max_items 30→50, max_articles 500→800
  - 新增 verify_gnews_params.py 验证脚本
"""
...
async def main_async():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Starting daily global fetch v4.6")
