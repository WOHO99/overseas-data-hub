...
v4.8.1: full_text max_articles=2000上限 + 跳过summary>200字符的文章
  - 解决v4.8全量抓取(~8355篇)超30min timeout被取消的问题
  - 按优先级降序截取Top2000，确保高优先级先抓
  - 已有summary>200字符的文章跳过(full_text边际收益低)
"""
...
async def main_async():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Starting daily global fetch v4.7")
    ...
    # v4.8.1: priority_filter="all" 全量抓取 + max_articles=2000 上限
    # 保守并发8，避免触发目标站反爬
    fetched, total_ft, ft_elapsed = await fetch_full_text_batch(
        articles_by_file_ft, priority_filter="all", concurrency=8, timeout=15, max_articles=2000
    )
