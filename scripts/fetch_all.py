...
v4.7.2: Playwright medium单变量测试(max_items=500)
  - 在v4.7.1基础上仅加Playwright medium pass，限500篇小样本
  - 目的：测量medium单篇耗时，推算全量时间，为拆Job决策提供数据
  - full_text保持high+medium不变
"""
...
async def main_async():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Starting daily global fetch v4.7.2 (Playwright medium sample test)", flush=True)
    ...
    # v4.7.2 单变量测试: Playwright medium pass (max_items=500, 小样本推算全量耗时)
    phase_log("=" * 70)
    phase_log("PHASE: Playwright GNews resolve (medium priority, SAMPLE=500)")
    ...
    # 推算全量耗时
    if pw_total_med > 0 and pw_elapsed_med > 0:
        per_article_s = pw_elapsed_med / pw_total_med
        # 从v4.7数据估算全量medium GNews URL数
        estimated_full = pw_total_med  # 如果500就是全部则无需外推
        phase_log(f"PHASE DONE: Playwright(medium sample) — {pw_resolved_med}/{pw_total_med} resolved ({pw_elapsed_med:.1f}s)")
        phase_log(f"  [BENCHMARK] Medium per-article: {per_article_s:.2f}s, sample={pw_total_med}")
    pw_resolved += pw_resolved_med
    pw_total += pw_total_med
    pw_elapsed += pw_elapsed_med
