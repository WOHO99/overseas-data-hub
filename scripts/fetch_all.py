...
v4.8: Playwright两轮(high→medium) + full_text全量抓取(>=0)
  - Playwright分两轮：先high(~60-70篇，3-5min) → 再medium(~100-200篇，8-12min)
  - full_text_batch降门槛至priority_filter="all"，全量抓取
  - common.py priority_map统一: {"high":10, "high+medium":3, "all":0}
  - 预期：canonical_url覆盖率83%→90%+，正文覆盖率5.6%→30-50%
"""
...
async def main_async():
    ...
    # v4.8: priority_filter="all" — 全量抓取所有文章正文
    # 保守并发8，避免触发目标站反爬
