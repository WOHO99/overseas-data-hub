#!/usr/bin/env python3
"""
fetch_all.py v4.8 — 主调度脚本 (subprocess隔离 + 并发模块 + Bucket+RapidFuzz去重)
...
v4.7: HTTP batch_resolve跳过(GNews HTTP解析0%覆盖，浪费10+min) + dedup_title_level naive/aware datetime修复
  ...
  - Pipeline顺序简化：modules → [SKIP HTTP batch] → Playwright batch → full_text → beijing → dedup → index
v4.8.2: full_text max_time=900s硬上限 + max_articles=2000 + skip summary>200
  - v4.8/v4.8.1均因full_text阶段耗时超workflow timeout被取消
  - max_time=900s(15min)确保full_text不会无限消耗时间，超时提前退出
  - 已抓取的成果仍保留，不影响后续beijing/dedup步骤
"""
...
async def main_async():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Starting daily global fetch v4.8.2")
    ...
    print("  [SKIP] HTTP batch_resolve disabled (0% GNews coverage, wastes 10+ min)")
    print("  Non-GNews sources already handled by feedburner_origlink/guid fallback in fetch_one()")

    resolved = 0  # skipped
    resolve_elapsed = 0

    # v4.5: Playwright 无头浏览器解析 GNews canonical_url
    # v4.8: 分两轮 — 先high(快速) → 再medium(覆盖面)
    # 纯HTTP batch_resolve对GNews覆盖率为0%，需JS渲染
    print(f"\n{'='*70}")
    print("Resolving GNews URLs with Playwright (Pass 1: high priority)...")
    ...
    # Pass 1: high priority only (~60-70篇, 3-5min)
    pw_resolved_hi, pw_total_hi, pw_elapsed_hi = await batch_resolve_gnews_with_browser(
        articles_by_file_pw, priority_filter="high"
    )

    # Pass 2: medium priority (~100-200篇, 8-12min)
    print(f"\n{'='*70}")
    print("Resolving GNews URLs with Playwright (Pass 2: medium priority)...")
    pw_resolved_med, pw_total_med, pw_elapsed_med = await batch_resolve_gnews_with_browser(
        articles_by_file_pw, priority_filter="high+medium"
    )

    pw_resolved = pw_resolved_hi + pw_resolved_med
    pw_total = pw_total_hi + pw_total_med
    pw_elapsed = pw_elapsed_hi + pw_elapsed_med
    print(f"  [PLAYWRIGHT SUMMARY] high={pw_resolved_hi}/{pw_total_hi}, medium={pw_resolved_med}/{pw_total_med}, total={pw_resolved}/{pw_total} ({pw_elapsed:.1f}s)")
    ...
    # v4.8: Batch fetch full text — 全量正文抓取（降门槛priority_filter="all"）
    print(f"\n{'='*70}")
    print("Fetching full text for ALL priority articles (v4.8: full coverage)...")
    ...
    # v4.8.2: priority_filter="all" 全量抓取 + max_articles=2000 + max_time=900s(15min)
    # 保守并发8，避免触发目标站反爬
    fetched, total_ft, ft_elapsed = await fetch_full_text_batch(
        articles_by_file_ft, priority_filter="all", concurrency=8, timeout=15, max_articles=2000, max_time=900
    )
