...
v5.0.1: 新增per-module full_text覆盖率报告
...
# v5.0.1: per-module full_text覆盖率报告
def log_fulltext_coverage(articles_by_file=None):
    """输出per-module full_text覆盖率报告，返回覆盖率摘要dict"""
    if articles_by_file is None:
        articles_by_file = load_articles_from_modules()

    phase_log("=" * 70)
    phase_log("FULL-TEXT COVERAGE REPORT")
    coverage_summary = {}
    grand_total = 0
    grand_with_text = 0
    ...
    for entry in MODULE_REGISTRY:
        output_file = entry["output"]
        mod_name = entry["module"]
        articles = articles_by_file.get(output_file, [])
        total = len(articles)
        with_text = sum(1 for a in articles if a.get("full_text"))
        pct = (with_text / total * 100) if total else 0
        if total > 0:
            phase_log(f"  {mod_name}: {with_text}/{total} ({pct:.1f}%)")
        coverage_summary[mod_name] = {
            "total": total,
            "with_full_text": with_text,
            "coverage_pct": round(pct, 1),
        }
        grand_total += total
        grand_with_text += with_text

    grand_pct = (grand_with_text / grand_total * 100) if grand_total else 0
    phase_log(f"  --- GLOBAL: {grand_with_text}/{grand_total} ({grand_pct:.1f}%) ---")
    coverage_summary["_global"] = {
        "total": grand_total,
        "with_full_text": grand_with_text,
        "coverage_pct": round(grand_pct, 1),
    }
    phase_log("=" * 70)
...
async def main_async():
    ...
    # v5.0.1: Full-text覆盖率报告
    coverage = log_fulltext_coverage(articles_by_file_ft)
    from common import atomic_write_json as _awj_cov2
    _awj_cov2(coverage, os.path.join(SCRIPT_DIR, "fulltext_coverage.json"))
...
async def finalize_mode():
    ...
    # 5.5 v5.0.1: Full-text覆盖率报告
    coverage = log_fulltext_coverage(articles_by_file_ft)
    from common import atomic_write_json as _awj_cov
    _awj_cov(coverage, os.path.join(SCRIPT_DIR, "fulltext_coverage.json"))
